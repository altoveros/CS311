...
{ 
 ...
 public: // prototypes to be used by the client
  ...
  //          otherwise, throws an exception - Underflow
  // PARAMETER: Parameter is the top element of the stack (pass by ref) 
  void topElem(el_t&);
  
  // ** Must add good comments for each function - See Notes1B

  //PURPOSE: Checks the stack if there are any elements, if not return true but if there are elements return false
  bool isEmpty();

  //PURPOSE: Checks the stack if is currently full (MAX IS 10) if so return true but if its not full return false
  bool isFull();

  //PURPOSE: Displays the current elements in the stack if there are any.
  void displayAll();

  //PURPOSE: Clears the whole stack and its elements if there are any.
  void clearIt();
